<?php
    ...
    class PagamentosController {
        ...
        public function verPagamento($request, $response, $args) {
            ...
            if (!$payment) {
                // Se o pagamento não for encontrado, retorne uma resposta 404 (Not Found)
                return $response->withStatus(404)->withJson(['message' => 'Pagamento não encontrado']);
            }

            // Se o pagamento for encontrado, retorne os detalhes como uma resposta JSON
        ...
        public function confirmarPagamento($response, $args) {
            ...
            // Atualize o status do pagamento para CANCELED no banco de dados
            $sql->atualizarStatusPagamento($id, 'CANCELED');
        
            // Retorne uma resposta 204 (No Content) indicando sucesso
